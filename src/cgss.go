...
func Login(args []string) int {
	...
	if err != nil {
		fmt.Println("Invalid Parameter: <exp> should be an intager ")
		return 0
	}
	player :=cg.NewPlayer()
	player.Name = args[1]
...
//将命令和处理函数对应
func GetCommdHandlers() map[string]func(args []string)int {
...
func main()  {
	fmt.Println("Casual Server Solution")
	startCenterService()
	Help(nil)
	r := bufio.NewReader(os.Stdin)
	handler := GetCommdHandlers()
	for { // 循环读取用户输入
		fmt.Print("Command>")
		b, _,_ := r.ReadLine()
		line := string(b)
		...
		if handler, ok := handler[tokens[0]]; ok {
			ret := handler(tokens)
			if ret != 0 {
				break
			} else {
				fmt.Println("Unkwon command:",tokens[0])
			}
		}
